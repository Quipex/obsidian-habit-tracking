    ...
    for (let w = 0; w < options.weeks; w++) {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        const cell = column.createDiv({
          cls: `dv-habitv2-cell ${isFuture ? "is-future" : ""} ${level ? `dot-l${Math.min(level, 4)}` : ""}`.trim()
        });
